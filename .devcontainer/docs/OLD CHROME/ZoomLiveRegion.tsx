...
export function ZoomLiveRegion(): JSX.Element {
  const { zoom, fitMode } = useCanvasAdapter();
  const [announcement, setAnnouncement] = useState<string>("");
  const timeoutRef = useRef<NodeJS.Timeout | null>(null);

  // Debounced announcement to avoid rapid-fire updates during smooth zoom
  const announceZoom = useCallback((newScale: number, newFitMode: CanvasFitMode): void => {
    ...
    requestAnimationFrame(() => {
      timeoutRef.current = setTimeout(() => {
        ...
        const message =
          newFitMode === "fit-width" ? `Fit to width` : `Zoom: ${percent} percent`;
